...
## 1st method to find and store the mean
## Using chaining functions to find the average of each variable for each activity and each subject
## Grouping the Extract tale with subject and activity, to find the mean of each quantity
Result <- Extract %>%
    group_by(subject, activity) %>%
    summarise_all(funs(mean))
write.table(Result, "Result.txt", row.name=FALSE)

## The Result table in stored in the working directory location

## 2nd method to find mean of each activity based on the subject only

tapply(Extract$activity, INDEX = Extract$subject, FUN = mean)

## 3rd method to find the mean for each variable for each activity and each subject

R <- aggregate(Extract, list(Extract$subject, Extract$activity), mean)